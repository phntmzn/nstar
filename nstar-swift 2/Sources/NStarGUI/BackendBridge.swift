...
struct ModelTrainResult: Decodable {
    let modelName: String
    let ok: Bool
    let epochs: Int?
...
struct TrainRequest {
    ...
    func bridgeArguments() -> [String] {
        ...
        if trainAllModels {
            arguments.append("--train-all-models")
        }
...
final class BackendBridge {
    ...
    func train(
        ...
        logHandler: @escaping (String) -> Void,
        completion: @escaping (Result<TrainResponse, Error>) -> Void
    ) -> Process? {
        runBridge(
            pythonPath: pythonPath,
            arguments: request.bridgeArguments(),
