...
from appconfig import DEFAULT_OUTPUT_DIR, app_base_dir
...
def run_generation(
    *,
    ...
    producers: int = max(multiprocessing.cpu_count() - 1, 1),
    ...
    producer_count = max(1, producers)
    resolved_name_prefix = name_prefix.strip() or model_name
    model_config = {
        ...
        "checkpoint_path": model_path.strip() or str(model_checkpoint_path(model_name, app_base_dir())),
        "force_retrain": retrain_model,
    }
    ...
    song_queue: Queue = multiprocessing.Queue(maxsize=max(1, queue_size))
    stop_event = multiprocessing.Event()
    producers = start_producers(song_queue, stop_event, producer_count, producer_model_config)

    try:
        return consume_and_save(song_queue, stop_event, output_dir, target, resolved_name_prefix)
