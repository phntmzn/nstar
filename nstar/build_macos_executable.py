...
def build_executable(
    *,
    ...
    model_path = PROJECT_ROOT / "progression_model.pt"
    if model_path.exists() and not skip_model_copy:
        destination = dist_dir / model_path.name
        shutil.copy2(model_path, destination)
        print(f"[build] copied model checkpoint to {destination}")
