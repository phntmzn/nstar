...
# --- Runtime paths and top-level constants ---
...
# --- Section plan and model bootstrap ---
...
def build_model_config(args: argparse.Namespace) -> dict[str, object]:
    # Generation always runs against one named progression model; if the caller
    # does not override the checkpoint path, use models/<model-name>.pt.
...
def prepare_progression_model(model_config: dict[str, object]) -> None:
    # Load or train the selected model before worker processes start so setup
    # errors happen once in the parent process instead of in every producer.
...
def generate_bar_rhythm() -> list[float]:
    ...
    # Fill the bar with weighted durations that never exceed the beats left.
    while remaining > 0:
...
def choose_degree(chord_name: str, previous_degree: int | None) -> int:
    ...
    # Bias toward chord tones, but occasionally step away from them so the line
    # does not collapse into a plain triad outline.
...
def choose_octave(note: str, previous_pitch: int | None) -> int:
    ...
    # Keep octave jumps small by choosing the closest absolute pitch.
...
# --- Phrase transforms and song assembly ---
...
def generate_section_phrase(
    ...
    if section.refresh or section.family not in motif_library:
        ...
        # A section family stores one generated motif that later sections can
        # reuse with transformations instead of regenerating from scratch.
...
def generate_song() -> dict:
    ...
    # Force a simple dominant-to-tonic cadence so every song resolves clearly.
    bars[-2]["chord"] = "C#m"
    bars[-1]["chord"] = "F#m"
...
# --- Deduplication and output paths ---
...
# --- Producer / consumer generation loop ---
def producer(
    song_queue: Queue,
    stop_event: Event,
    seed_value: int,
    ...
    # Each worker loads the same named model checkpoint, then generates songs
    # independently until the writer signals shutdown.
    configure_progression_model(**model_config)
    get_progression_model()
...
def consume_and_save(
    ...
    while saved < target:
        ...
        # Deduplicate in-memory songs so worker collisions do not produce
        # duplicate MIDI files during the same run.
...
def main() -> None:
    ...
    # Workers reuse the already-prepared checkpoint silently; only the parent
    # process prints model load/train status.
