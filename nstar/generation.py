...
DEFAULT_MODEL_PATH = Path(__file__).resolve().parent / "progression_model.pt"
...
def build_model_config(args: argparse.Namespace) -> dict[str, object]:
    checkpoint_path = args.model_path.strip() or None
    return {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Torch seed used when training the progression model.",
    )
    parser.add_argument(
        "--train-log-every",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Checkpoint path for the trained progression model.",
    )
    parser.add_argument(
        "--retrain-model",
        action="store_true",
