...
from neuralnetwork import generate_progression_template
...
DEFAULT_BPM = 157
BEATS_PER_BAR = 4
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent / "generated_midis" # <-- date time
DEFAULT_QUEUE_SIZE = 256
DEFAULT_NAME_PREFIX = "phntmzn"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Filename prefix used for saved MIDI files.",
    )
...
def producer(song_queue: Queue, stop_event: Event, seed_value: int) -> None:
...
def start_producers(
    ...
    for seed in range(producer_count):
        process = multiprocessing.Process(
            target=producer,
            args=(song_queue, stop_event, seed),
        )
        process.start()
...
def main() -> None:
    ...
    producers = start_producers(song_queue, stop_event, producer_count)

    try:
        consume_and_save(song_queue, stop_event, output_dir, args.target, args.name_prefix)
